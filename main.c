...
void preset(){
	...
	// откомментить две строчки ниже для части с прерываниями, закомментить, если вручную проверяем флаги
    ...
    while (1) {
		...
		if (start_program) {
			while (1) {
				process_states();
				// PORTC = ~portc;
				// можно дописать фрагмент кода, который будет в прерывании попеременно гасить все диоды / зажигать согласно переменной portc
				// либо попеременно зажигать/гасить диоды процессов посредством использования битовой маски 0b00111000 
				}}
